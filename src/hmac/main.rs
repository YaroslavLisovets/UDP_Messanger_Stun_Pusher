...
fn generate_auth_key(socket_id: &str, channel_name: &str, user_data: &str) -> String {
...
fn generate_message(socket_id: &str) -> String {
...
fn main() {
    generate_message("172160.853622");
    let subject = r#"172160.853622:presence-channel:{"user_id": "159.89.173.104"}"#;
    let subject = subject.to_string();
    let subject = subject.as_bytes();

    let mut hmac = HmacSha256::new_from_slice(env!("pusher_secret").as_ref()).unwrap();
    hmac.update(subject.as_ref());
    let result = hmac.finalize();
    println!("{}", encode(result.into_bytes()));
}